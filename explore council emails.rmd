...
```{r}
data_clean %>% 
  mutate(VIPs = case_when(ToCCM ~ "City Council Member",
                          FromCCM ~ "City Council Member",
                          FromCO ~ "City Official",
                          SentTo == "Council" ~ "Offical Council Email Address")) %>% 
  select(SentFrom, SentTo, VIPs) %>% 
  separate_rows(SentTo, sep = ",") %>% 
  filter(str_detect(SentTo, "\\[.*\\]")) %>% 
  mutate(SentTo = str_trim(SentTo, side = "both")) %>% 
  as_tbl_graph() %>% 
  ggraph(layout = 'kk') +
  geom_edge_link(aes(color = VIPs)) + 
  geom_node_point()
```
