...
raw_data <-
  bind_rows(raw_data2019, raw_data2020)
...
*edit* Since initially performing this analysis, the COVID-19 Pandemic has swept the globe and thrown data project everywhere, this included, into disarray. It is somewhat apparent that engagement has increased in a large way.
...
For this iteration, we are using a simple "bag of words" approach using Latent Derichlet Allocation. This is a relatively simplistic language model that builds off of other Bayesian methods but does not conflate location in a string with significance. 
...
clean_unnest <- data_clean %>% 
  #sample_n(5000) %>% 
  unnest_tokens(word, PlainTextBody) %>% 
  anti_join(stop_words, by = c("word")) %>% 
  filter(!str_detect(word, '[0-9]+'),
         !(word %in% c('http' , 'https', 'tinurl\\.com'))) %>% 
  mutate(word_stemmed = SnowballC::wordStem(word, "english") ) %>%
  filter(!(word_stemmed %in% c('boulder', 'citi', 'council'))) %>% 
  group_by(word_stemmed) %>% 
  arrange(word) %>% 
  mutate(word_cleaned = first(word)) %>% 
  ungroup() %>% 
...
The perplexity plots I built showed that 40 topics has the best fit. This makes sense to me but is very hard to delineate and build a narrative around. Since we have no clear topics in mind, reducing that number will help down the road.
...
emails_topics <- data_clean %>% 
  left_join(topic_designation, by = c('MessageIdentifier' = 'document'))
```
